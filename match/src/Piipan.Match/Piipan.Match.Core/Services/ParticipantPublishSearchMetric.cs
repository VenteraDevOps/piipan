...
namespace Piipan.Match.Core.Services
{
    public class ParticipantPublishSearchMetric : IParticipantPublishSearchMetric
    {
        ...
        private void InitializeEventGridPublisherClient()
        {
            try
            {
                if (_client == null)
                {
                    ...
                    {
                        _logger.LogWarning("EventGridMetricSearchEndPoint and EventGridMetricSearchKeyString environment variables are not set.");
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <returns></returns>
        public Task PublishSearchdMetric(ParticipantSearchMetrics metrics)
        {
            try
            {
