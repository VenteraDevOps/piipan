...
Because of the performance test results, in addition to the other concerns listed above, we plan to enhance the performance of the bulk upload process. We considered a few options for improving performance- Native PSQL COPY function, Azure Batch, Azure Storage Queue, Azure Service Bus, Azure Data Factory, Durable Functions, & Azure Event Hub. 

Using the PSQL Copy functionality gives a signficant performance boost over a batch insert. Testing demonstrated it was orders of magnitude faster (17s for 100k records vs 30minutes). However, it didn't scale linearly and struggled to complete successfully for 500k records or more. The larger the upload, the more frequently timeouts occurred. In order to get more reliable performance, the upload had to be broken apart into batches and the Connection & Command KeepAlive & Timeout settings had to be tweaked in order to support larger uploads. Even then, it was error prone and would take hours for millions of records. Processing times of this length would still cause Azure timeouts. The error-prone nature of this solution also would result in retries, causing potentially duplicate records. Between the timeouts and retries, this approach would still require rearchitecting to accommodate idempotency and long running tasks.  Additionally, this approach does not help with setting up future analysis streams.
...
Azure Data Factory is fast. Our preliminary tests with it showed it can upload one million records in less than 10 minutes. It could likely be tweaked to process even more quickly. It would also operate asynchronously allowing an Azure function calling it to return immediately, avoiding Azure Event Grid & Azure Function timeouts. Data is inserted all at once upon completion of the pipeline (rather than continously inserted record-by-record or batch-by-batch). The issue is that the complexity of using ADF is a large downside. ADF acts as an asynchronous, black-box once called. We would need to develop code to configure, trigger, monitor, and retrieve run & error details from ADF. Insight into its completion, success/failure, error details, etc. would involve a polling mechanism (https://docs.microsoft.com/en-us/azure/data-factory/monitor-programmatically). Also, we still needed to configure Connection & Command KeepAlive & Timeout settings to allow pipelines to run successfully. Additionally, this approach does not help with setting up future analysis streams.
...
### Some Challenges
While providing significant performance gains, decoupling the csv parsing from Postgres record persistence presents a couple challenges regardless of the approaches described above. Participants committed in batches (or even record by record) mean partial uploads may exist. We currently only perform matches against the most recent uploads and we clean-up/remove the data from old uploads. So we need to consider 

#### 1. Progress monitoring and how to distinguish between successful uploads vs partial/failed uploads?
As a means of monitoring progress, we could track upload participant count as part of upload record. We could record the number of participants belonging to the upload when we create the upload record. Then we could check/monitor progress by issue a count-distinct query against participants. This approach allows us to calculate how many participants have been uploaded at any given time. Duplicates with the csv could be problematic. If we find it's typical that they exist, we may need to send duplicates to an error queue or modify the monitoring query not to be distinct. In the future, we could provide finer grain error reporting by potentially move error events to error queues, set up receivers to consume them, and then provide more detailed error information to users.
