...
namespace Piipan.Etl.Func.BulkUpload.Tests
{
    public class BulkUploadTests
    {
        ...
        [Fact]
        public async void Run_BlankEvent()
        {
            // Arrange
            var participantApi = Mock.Of<IParticipantApi>();
            ...
            var storageBlobEvent = Mock.Of<StorageBlobCreatedEventData>();
            var eventTrigger = Mock.Of<EventGridEvent>( m => m.Data == storageBlobEvent);

            // Act 
            await function.Run(eventTrigger.ToString(), logger.Object);
